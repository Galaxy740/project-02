            ...
            <div class="col-sm-10">
                <form method="post" action="">
                    <div class="form-group">
                        ...
                        <input type="datetime-local" class="form-control" id="date_picker" name="date_picker"
                               value="<?= $row_flight['date_departure'] ?>">
                    </div>
                    <div class="form-group">
                        <label for="product_name">Purchased seats</label>
                        <input type="text" class="form-control" id="purchased_seats" name="purchased_seats"
                               value="<?= $row_flight['purchased_seats'] ?>">
                    </div>
                    <div class="form-group">
                        <label for="product_name">Flight code</label>
                        <input type="text" class="form-control" id="flight_code" name="flight_code"
                               value="<?= $row_flight['flight_code'] ?>">
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-control" id="destination" name="destination_id">
                            <?php if (mysqli_num_rows($destination_result) > 0) { ?>
                                ...
                                <?php while ($row = mysqli_fetch_assoc($destination_result)) { ?>
                                    ...
                                    <option value="<?= $row['destination_id']; ?>" <?php if ($row['destination_id'] == $row_flight['destination_id']) {
                                        echo 'selected';
                                    } ?> ><?= $row['destination_point'] ?></option>
